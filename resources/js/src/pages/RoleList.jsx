...
const RoleList = () => {
  ...
  const groupedPermissions = permissions?.reduce((acc, perm) => {
    const group = perm.group || (t('admin.roles.uncategorized'));
  ...
  const handleBulkAction = async (action) => {
    if (action === 'delete' && selectedIds.length > 0) {
      if (window.confirm(t('admin.roles.delete_roles_confirm', { count: selectedIds.length }))) {
        try {
  ...
  const handleDelete = async (id) => {
    if (window.confirm(t('admin.roles.delete_role_confirm'))) {
      try {
    ...
    <div className="space-y-6 pb-10" dir={t('auto.ltr')}>
      ...
      <div className="flex gap-4">
          <div className="flex-1">
            <StatsCard title={t('admin.roles.total_roles')} value={roles?.length || 0} icon={faUserShield} color="purple" />
          </div>
          <div className="flex-1">
            <StatsCard title={t('admin.roles.total_permissions')} value={permissions?.length || 0} icon={faKey} color="blue" />
          </div>
      </div>
            ...
            <table className="w-full text-sm text-start">
              <thead className="bg-slate-50 border-b border-slate-200 text-slate-600 font-bold uppercase tracking-wider text-[10px]">
                <tr>
                  ...
                  <th className="px-6 py-4 text-start">{t('admin.roles.role_and_desc')}</th>
                  <th className="px-6 py-4 text-start">{t('admin.roles.permissions')}</th>
      ...
      <Modal
        ...
        title={editMode ? (t('admin.roles.edit_role')) : (t('admin.roles.add_new_role'))}
        size="lg"
        footer={
          <div className="flex justify-end gap-2 w-full">
            <Button variant="secondary" onClick={() => setIsModalOpen(false)}>{t('common.cancel')}</Button>
            <Button variant="primary" onClick={handleSubmit} disabled={isCreating || isUpdating}>
              {(isCreating || isUpdating) ? (t('common.saving')) : (t('admin.roles.save_role'))}
            </Button>
          </div>
        }
      >
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
              <FontAwesomeIcon icon={faKey} /> {t('admin.roles.assign_permissions')}
            </label>
