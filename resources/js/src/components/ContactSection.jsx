...
const CourseCTA = ({ lang, onNavigate }) => {
    ...
    return (
        ...
        hover:border-emerald-400/60 transition-all duration-300 shadow-lg p-4 text-start"
            aria-label={
                isRtl ? "سجل في الدورة التالية" : "Register for Next Cohort"
            }
        >
            ...
            {/* Icon */}
            <div
                className="relative w-11 h-11 rounded-xl bg-emerald-500/20
        ...
        group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300"
            ...
            <div className="relative flex-1 min-w-0">
                ...
                <p className="text-white font-black text-sm md:text-base leading-tight">
                    {isRtl
                        ? "سجل في الدورة القادمة واحجز مكانك"
...
const CustomSelect = ({
    ...
    const selectedLabel = options.find(
        (o) => o === value || (typeof o === "object" && o.value === value),
    );
    const displayLabel =
        ...
        <div>
            ...
            <div className="flex flex-wrap gap-2 mt-1">
                {c.companyInterestOptions.map((opt) => {
                    const checked = (fields.companyInterest || []).includes(
                        opt,
                    );
                    return (
                        <label
    ...
    <div className="space-y-3 animate-fade-in">
        ...
        <label className="flex items-center gap-2 cursor-pointer group">
            <input
                ...
                onChange={(e) =>
                    setFields((f) => ({ ...f, wantsDemo: e.target.checked }))
                }
            />
...
const TrainerFields = ({ c, fields, setFields }) => (
    <div className="space-y-3 animate-fade-in">
        ...
        <CustomSelect
            ...
            onChange={(val) =>
                setFields((f) => ({ ...f, trainerInterest: val }))
            }
        />
    </div>
);
...
const ContactSection = ({ t, lang, onNavigate }) => {
    ...
    useEffect(() => {
        if (form.audienceType) {
            trackEvent("contact_form_select_audience", {
            ...
            conditionalFields:
                form.audienceType === "company"
                    ...
                    : form.audienceType === "trainee"
                      ? {
                            track: condFields.track,
                            wantsDemo: condFields.wantsDemo,
                        }
                      : {
