...
const ICON_MAP = {
    ...
    login: { Ic: LogIn, bg: "bg-slate-100", fg: "text-slate-700" },
};

// ─── MegaMenu Panel ──────────────────────────────────────────────────────────
const MegaMenu = ({ t, onLinkSelect, onClose }) => {
    ...
    return (
        <motion.div
            ...
            style={{
                background:
                    "linear-gradient(135deg, #0f172a 0%, #1e3a5f 60%, #0d4f4f 100%)",
            }}
        >
...
const Navbar = ({
    ...
    const handleLogoClick = () => {
        if (location.pathname !== "/" || isCourseView) {
            navigate("/");
            return;
        }
    ...
    const renderMobileIcon = (iconName) => {
        const meta = ICON_MAP[iconName] || {};
        const Icon = meta.Ic;
        return Icon ? <Icon className="w-4 h-4" strokeWidth={1.8} /> : null;
    };
                    ...
                    <div className="hidden md:flex items-center gap-3">
                        ...
                        {showTopActions && (
                            <button
                                ...
                                className={`px-4 py-2 rounded-lg text-sm font-bold shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-emerald-400 ${isCourseView ? "bg-emerald-500 text-white cursor-default" : "bg-gradient-to-r from-deep-blue to-emerald text-white"}`}
                                data-key="nav_cta"
                            >
                                {isCourseView
                                    ? lang === "ar"
                        ...
                        <button
                            ...
                            className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-emerald-400 ${megaOpen ? "bg-gray-100 text-deep-blue" : "text-gray-600 hover:bg-gray-50 hover:text-deep-blue"}`}
                        >
                            {t.nav?.home
                                ? lang === "ar"
                                    ? "القائمة"
