    ...
    <div className="bg-white p-3 rounded-xl shadow-sm border border-slate-200 flex flex-col md:flex-row items-center justify-between gap-3">
      ...
      <div className="flex items-center gap-2 w-full md:w-auto">
        <div className="relative flex-1 md:w-64">
          <span className={`absolute inset-y-0 flex items-center text-slate-400 ${t('auto.left_3')}`}>
            <FontAwesomeIcon icon={faSearch} className="text-xs" />
          </span>
          <input
            type="text"
            className={`block w-full py-1.5 text-xs border border-slate-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-500 bg-slate-50/50 ${t('auto.pl_9_pr_3')}`}
      ...
      <div className="flex items-center gap-3 w-full md:w-auto justify-end">
        ...
        <div className="relative">
          <select 
            ...
            value=""
          >
            <option value="" disabled>{t('auto.bulk_actions')}</option>
            <option value="delete" className="text-red-600">{t('auto.delete_selected')}</option>
            <option value="activate">{t('auto.activate')}</option>
          </select>
          <div className={`pointer-events-none absolute inset-y-0 flex items-center px-2 text-slate-400 ${t('auto.right_1')}`}>
