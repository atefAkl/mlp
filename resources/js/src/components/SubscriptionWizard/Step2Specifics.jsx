                                    ...
                                    {isSelected && (
                                        ...
                                        >
                                            <div className="flex justify-between items-center mb-3">
                                                <p className="text-white text-[13px] font-bold drop-shadow-sm">{prog.target}</p>
                                                <div className="bg-emerald-500 text-white font-bold text-[11px] px-3 py-1 rounded-full whitespace-nowrap shadow-sm">
                                            ...
                                            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
                                                <div className="flex items-center gap-2 text-[12px] text-white/90">
                                                    ...
                                                    <span className="font-medium">{w.specifics?.start} {prog.startDate}</span>
                                                </div>
                                                <div className="flex items-center gap-2 text-[12px] text-white/90">
                                                    <Clock className="w-4 h-4 text-emerald-300" />
                                                    <span className="font-medium">{prog.duration.replace("أسبوع", w.specifics?.weeks || "أسبوع")}</span>
                                                </div>
                                                <div className="flex items-center gap-2 text-[12px] text-white/90">
                                                    <Users className="w-4 h-4 text-emerald-300" />
                                                    <span className="font-medium">{w.specifics?.seats} {prog.seats}</span>
                                                </div>
                                                {prog.certificate && (
                                                    <div className="flex items-center gap-2 text-[12px] text-white/90">
                                                        <Award className="w-4 h-4 text-emerald-300" />
                                                        <span className="font-medium">{w.specifics?.cert}</span>
                                                    </div>
                                                )}
                                            </div>

                                            <div className="space-y-2">
                                                <div className="text-[12px] text-white font-bold flex items-center gap-2">
                                                    <Code className="w-4 h-4 text-emerald-300" /> {w.specifics?.techs}
                                                </div>
                                                <div className="flex flex-wrap gap-1.5 pb-1">
                                                    {prog.tech.map(t => <span key={t} className="bg-white/20 text-white font-medium border border-white/30 shadow-sm text-[11px] px-2.5 py-1 rounded-md">{t}</span>)}
                                                </div>
                                                
                                                <div className="text-[12px] text-white font-bold flex items-center gap-2 mt-2">
                                                    <ListChecks className="w-4 h-4 text-emerald-300" /> {w.specifics?.reqsLabel}
                                                </div>
                                                <div className="flex flex-wrap gap-1.5">
                                                    {prog.requirements.map(r => <span key={r} className="bg-emerald-500/40 text-white font-medium border border-emerald-400/50 shadow-sm text-[11px] px-2.5 py-1 rounded-md">{r}</span>)}
                                                </div>
                                            </div>
                                        </motion.div>
                                    )}
                                    ...
                                    {isSelected && (
                                        ...
                                        >
                                            <div className="flex flex-wrap gap-4 mb-3">
                                                <div className="flex items-center gap-1.5 text-[12px] text-white font-medium bg-white/10 px-3 py-1.5 rounded-lg border border-white/20">
                                            ...
                                            <div className="text-[13px] text-white/90 leading-relaxed bg-black/30 p-4 rounded-xl border border-white/10 shadow-inner">
                                                <span className="text-emerald-300 font-bold flex items-center gap-2 mb-2">
                                                    <ListChecks className="w-4 h-4" />
                                                    المتطلبات:
                                                </span>
                                                <p className="font-medium text-white">{job.requirements}</p>
                                            </div>
                                        </motion.div>
                                    )}
                                    </AnimatePresence>
